...
export function EntryView(props: Props) {
  ...
  if (mode === "form") {
    return <EntryForm {...props} onBack={() => setMode("upload")} />;
  }

  return <UploadView branchName={props.branchName} onManual={() => setMode("form")} />;
}
    ...
    <div className="min-h-screen">
      <div className="p-6 lg:p-8 space-y-6 mx-auto w-full max-w-[820px]">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-ink leading-tight">{branchName}</h1>
        ...
        <div className="card p-5 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <p className="font-bold text-ink">{t("entry.manual")}</p>
            <p className="text-xs text-ink-soft">{t("entry.manualHint")}</p>
          </div>
          <button onClick={onManual} className="btn-primary shrink-0">
            {t("entry.manualOpen")}
          </button>
        </div>
      </div>
